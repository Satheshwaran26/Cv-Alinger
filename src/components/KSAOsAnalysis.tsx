        ...
        {items.map((item, index) => (
          <div key={index} className="glass rounded-lg p-4 transition-all hover:shadow-sm">
            ...
            <div className="space-y-2">
              ...
              <div className="relative pt-1">
                <div className="flex items-center justify-between">
                  <div className="text-xs text-muted-foreground">Job Requirement</div>
                  <div className="text-xs font-medium">{item.jobReqScore}%</div>
                </div>
                <div className="overflow-hidden h-2 text-xs flex rounded bg-gray-200 dark:bg-gray-700 mt-1">
                  <div
                    className="bg-primary h-full rounded transition-all duration-500 ease-out"
                    style={{ width: `${item.jobReqScore}%` }}
                  ></div>
                </div>
              </div>
            </div>
